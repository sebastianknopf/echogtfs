#!/usr/bin/env bash
# dbcmd — open an interactive psql session against the database PostgreSQL container.
#
# Usage:
#   ./scripts/dbcmd.sh                                  # interactive session
#   ./scripts/dbcmd.sh "SELECT count(*) FROM {table};"  # single command, then exit
#
# The script reads POSTGRES_USER, POSTGRES_DB, and POSTGRES_PASSWORD from the
# .env file in the repository root and connects to the running database
...
CONTAINER_NAME="echogtfs-database-1"
